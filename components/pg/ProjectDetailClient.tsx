...
interface Props {
  project: PGProject;
  initialPayments: PGPayment[];
  initialContractors: PGContractor[];
  initialMilestones: PGMilestone[];
}
...
export function ProjectDetailClient({
  ...
  const refreshPayments = useCallback(async () => {
    const res = await fetch(`${API}/payments?project_id=${p.id}`, { credentials: "include" });
    if (res.ok) setPayments(await res.json());
  }, [p.id]);

  const refreshMilestones = useCallback(async () => {
    const res = await fetch(`${API}/milestones?project_id=${p.id}`, { credentials: "include" });
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-200">
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-neutral-400 uppercase tracking-wider">Date</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-neutral-400 uppercase tracking-wider">Vendor</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-neutral-400 uppercase tracking-wider">Category</th>
                  <th className="text-right px-4 py-2.5 text-xs font-semibold text-neutral-400 uppercase tracking-wider">Amount</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-neutral-400 uppercase tracking-wider">Notes</th>
                </tr>
              </thead>
              <tbody>
                {payments.map((pay) => (
                  <tr key={pay.id} className="border-b border-neutral-100 last:border-0">
                    <td className="px-4 py-2.5 text-neutral-600">{pay.payment_date || "--"}</td>
                    <td className="px-4 py-2.5 text-neutral-900 font-medium">{pay.vendor}</td>
                    <td className="px-4 py-2.5 text-neutral-600">{pay.category}</td>
                    <td className="px-4 py-2.5 text-right font-mono font-semibold text-neutral-900">{fmtFull(Number(pay.amount))}</td>
      ...
      {/* Milestones */}
      <div>
        <h2 className="text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">Milestones</h2>
        <div className="space-y-2">
          {milestones.map((ms) => {
            const styles = STATUS_STYLES[ms.status] || STATUS_STYLES.pending;
            return (
              <button
                key={ms.id}
              ...
              >
                ...
                <div className="flex-1">
                  <span className="text-sm font-medium text-neutral-900">{ms.name}</span>
                  {ms.phase && (
      ...
      {/* Contractors */}
      <div>
        <h2 className="text-xs font-bold uppercase tracking-wider text-neutral-400 mb-3">Contractors</h2>
        <div className="space-y-2">
          {contractors.map((c) => (
            <Card key={c.id}>
      ...
      <CardContent className="p-4">
        <form onSubmit={submit} className="space-y-3">
          <div className="flex gap-2 flex-wrap">
            <input className={inputClass} placeholder="Vendor" value={vendor} onChange={(e) => setVendor(e.target.value)} required />
            <input className={`${inputClass} w-[120px]`} type="number" step="0.01" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} required />
          </div>
          <div className="flex gap-2 flex-wrap">
