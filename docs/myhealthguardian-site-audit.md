...
## Lovable‑Ready Master Prompt (copy/paste)

**Prompt:**

You are refactoring the My Health Guardian experience to be ultra‑intuitive for kids and older adults. Produce a clean, modern UI with large, friendly typography, fewer steps to start, and obvious next actions. Apply the following changes across the landing page **and** app UI where applicable:

1) **Align the brand promise**
- Update the hero headline to explicitly connect “Health Guardian” with meal planning and wellness (e.g., “Your family’s health‑first meal planner”).
- Add a one‑sentence subheading that explains the outcome in plain language.

2) **Reduce decision fatigue**
- Keep exactly one primary CTA (“Build My First Week”).
- Add one secondary CTA (“See a 60‑second demo”).
- Remove or demote repeated CTAs throughout the page.

3) **Make “How It Works” visual and concrete**
- Use a 3‑card flow that shows the exact inputs (diet, budget, time, allergies, cooking skill).
- Display a 7‑day calendar preview and a grocery list preview beside the steps.
...
5) **Clarify Rosie (AI assistant)**
- Replace abstract “Chill/Activated Guardian Mode” with concrete choices (Budget, High Protein, Picky Eaters, Diabetes‑Friendly).
- Add a short “Rosie helps you…” list (3 bullets) with a friendly avatar.

6) **Compress the story section**
- Replace the long dinner‑drama narrative with a 3‑bullet summary and a single CTA below it.

7) **Pricing clarity**
- Add a “Most Popular” badge.
- Include 3–5 bullet benefits per plan.
- Show “Cancel anytime / No risk” directly under the primary CTA.

8) **Navigation clarity**
- If users are not logged in, show only “Get Started” and “Login” in the top nav.
- If UI chips or buttons are mockups, label them as “Preview.”

9) **Accessibility**
- Increase base font size to 18px, ensure AA contrast, and make all tap targets ≥ 44px tall.
- Use plain‑language labels and avoid idioms like “magic wishes.”

10) **App‑level UX fixes (implementation)**
- **Bottom nav “Log”**: ensure it opens the meal logging flow directly (meal scanner/log entry), not Settings.
- **Quick Actions**: add visible click feedback (scale/ring) and loading states for any action that triggers async work.
- **Mood/shortcut buttons**: add a brief loading spinner/disabled state while the message is being sent so users know it worked.
- **Nav/controls accessibility**: add `aria-label` values to all icon‑only buttons.

Deliver a cohesive, low‑friction experience that a 10‑year‑old or 75‑year‑old can use without instructions.
